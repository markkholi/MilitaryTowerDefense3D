...
const { AntiTankModels } = await load('anti-tank-model');
...
const assetUrl = new URL('../public/assets/anti-tank/anti-tank-turret.glb', import.meta.url);
...
function viewHarness(antiTanks) {
  ...
  Object.assign(view, {
    ...
    entities: new Map(), effectObjects: new Map(), scene: new THREE.Scene(), workshop: new UnitWorkshop(), antiTanks,
...
test('the supplied anti-tank asset retains indexed geometry, an independent gun rig and complete material maps', async () => {
  ...
  for (const mesh of json.meshes) for (const primitive of mesh.primitives) {
    ...
    for (const binding of [pbr.baseColorTexture, pbr.metallicRoughnessTexture, material.normalTexture, material.occlusionTexture].filter(Boolean)) {
      ...
      assert.ok(image.uri && !image.uri.startsWith('data:') && !image.uri.includes('..'), 'material texture is packaged beside the asset');
  ...
  assert.equal(triangles, 2870, 'all triangles from the supplied Collada asset survive conversion');
  let gunTriangles = 0;
  recoil.traverse(node => { if (node.isMesh) gunTriangles += node.geometry.index.count / 3; });
  assert.equal(gunTriangles, 1660, 'the complete supplied gun assembly participates in recoil');
  disposeModelResources(scene);
});

test('anti-tank clones share resources while each turret, recoil and muzzle flash moves independently', async () => {
  const models = new AntiTankModels(); await models.load(8, { loadAsync: readGeometry });
  const first = models.create(), second = models.create();
  assert.ok(first.root.userData.antiTank && second.root.userData.antiTank);
...
test('anti-tank loading preserves AO and surface detail and disposes shared maps and bitmaps once', async () => {
  const fixture = fakeAsset(), models = new AntiTankModels();
...
test('anti-tank failures preserve a playable fallback and malformed or late assets release resources', async t => {
  t.mock.method(console, 'warn', () => {});
  const models = new AntiTankModels();
  await models.load(4, { loadAsync: async () => { throw new Error('asset unavailable'); } });
  assert.equal(models.ready, false); assert.equal(models.create(), null);
  const view = viewHarness(models), fallback = view.entity('p1', 'at', false, 0, { x: 420, y: 150 });
  assert.ok(fallback.model.root.children.length > 0 && !fallback.model.root.userData.antiTank);
  ...
  const late = new AntiTankModels(), fixture = fakeAsset(); let resolve;
...
test('anti-tank late replacement and upgrades preserve off-road placement, aim and camera picking', async () => {
  const models = new AntiTankModels(), view = viewHarness(models), point = { x: 420, y: 150 };
  const state = { buildFamily: 'at', pointer: point, positions: [] };
  assert.deepEqual(view.placementPreview(state), { point, valid: true }, 'anti-tank placement uses the chosen off-road location');
  assert.equal(view.placementPreview({ ...state, pointer: { x: 100, y: 128 } }).valid, false, 'anti-tank cannot occupy the road');
  assert.equal(view.placementPreview({ ...state, positions: [{ family: 'at', ...point }] }).valid, false, 'units cannot overlap');
  const initial = view.entity('p7', 'at', false, 0, point).model; initial.heading.rotation.y = .55;
  await models.load(4, { loadAsync: readGeometry });
  const replacement = view.entity('p7', 'at', false, 0, point).model;
  assert.ok(replacement.root.userData.antiTank); assert.equal(initial.root.parent, null);
  assert.equal(replacement.heading.rotation.y, .55);
  for (const rank of [0, 1, 3]) {
    const position = { id: 7, family: 'at', rank, ...point, angle: 1.2 };
    view.render(gameWith(position)); const model = view.entities.get('p7').model;
    assert.ok(model.root.userData.antiTank); assert.equal(model.heading.rotation.y, .55);
...
test('anti-tank aiming leaves its base fixed and its tracer starts at the recoiling muzzle', async () => {
  const models = new AntiTankModels(), view = viewHarness(models); await models.load(4, { loadAsync: readGeometry });
  const position = { id: 7, family: 'at', rank: 0, x: 420, y: 150, angle: -.7 };
  ...
  const shot = { id: 71, type: 'shot', weapon: 'at', sourceId: 7, x: position.x, y: position.y, tx: 650, ty: 220, age: 0, duration: .16 };
  ...
  assert.ok(origin.distanceTo(firedMuzzle) < 2e-5, 'anti-tank tracer starts at the aimed muzzle instead of the unit center');
